...
public class BookmarkService {
	...
	public List<ArticleSummaryProjection> getArticleListByUserId(Long userId){
		List<ArticleSummaryProjection> articleProjections = new ArrayList<>();
		List<Bookmark> bookmarkList = bookmarkRepository.findAllByUserId(userId);
		for (Bookmark bookmark: bookmarkList) {
			ArticleSummaryProjection projection = articleRepository.findArticleSummaryProjectionById(bookmark.getArticle().getId());
